...
export async function POST(req: NextRequest) {
    try {
        ...
        console.log([institution, subjects, location, year, sampleTeachingVideo, subjectToTeach, name, email]);
        
        if (!institution || !subjects || !location || !year || !sampleTeachingVideo || !subjectToTeach || !name || !email) {
            return NextResponse.json({ msg: "All fields are required",success:false }, { status: 400 })
        }
        if (!(sampleTeachingVideo instanceof File)) {
            return NextResponse.json({ msg: "Invalid file upload",success:false }, { status: 400 });
        }
        ...
        return NextResponse.json({ msg: "Tutor created successfully",tutor, success:true }, { status: 201 })
    } catch (error: any) {
        console.log(error);
